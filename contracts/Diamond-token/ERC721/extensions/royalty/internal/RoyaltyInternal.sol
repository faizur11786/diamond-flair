...
import "./../interfaces/IRoyaltyInternal.sol";
...
abstract contract RoyaltyInternal is IRoyaltyInternal {
    ...
    function _setTokenRoyalties(
        ...
        for (uint256 i = 0; i < royaltyConfigs.length; i++) {
            ...
            if (royaltyConfig.recipient == address(0)) {
                ...
                l.tokenRoyalties[royaltyConfig.tokenId] = TokenRoyalty(
    ...
    function _setDefaultRoyalty(TokenRoyalty memory royalty) internal virtual {
        ...
        RoyaltyStorage.layout().defaultRoyalty = TokenRoyalty(
            royalty.recipient,
            royalty.bps
        );
     ...
     */
    function _getTokenRoyaltyByIndex(
        uint256 index
    ) internal view virtual returns (TokenRoyaltyConfig memory) {
        RoyaltyStorage.Layout storage l = RoyaltyStorage.layout();
    ...
    function _royaltyInfo(
        ...
        if (l.tokenRoyalties[tokenId].recipient != address(0)) {
            return (
                l.tokenRoyalties[tokenId].recipient,
                (value * l.tokenRoyalties[tokenId].bps) / 10000
            );
        }

        if (
            l.defaultRoyalty.recipient != address(0) &&
            l.defaultRoyalty.bps != 0
        ) {
            return (
                l.defaultRoyalty.recipient,
                (value * l.defaultRoyalty.bps) / 10000
            );
        }
