...
import "./internal/RoyaltyInternal.sol";
...
contract Royalty is IRoyalty, RoyaltyInternal {
    ...
    function defaultRoyalty()
        external
        view
        virtual
        returns (TokenRoyalty memory)
    {
     ...
     */
    function getTokenRoyaltiesCount()
        external
        view
        virtual
        override
    ...
    function getTokenRoyaltyByIndex(
        uint256 index
    ) external view virtual override returns (TokenRoyaltyConfig memory) {
    ...
    function getRaribleV2Royalties(
        uint256 tokenId
    ) external view override returns (IRaribleV2.Part[] memory result) {
